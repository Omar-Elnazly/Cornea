...
class ORTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        sys.path.insert(0, 'F:\\Cornea\\ORTrack')
    ...
    def track(self, image, info: dict = None):
            ...
            if not self.use_visdom:
                ...
                while self.pause_mode:
                    if self.step:
                        self.step = False
                        break
            best_score = float(response.max().item())
        ...
        if self.frame_id % UPDATE_INTERVAL == 0:
            if UPDATE_MIN < best_score < UPDATE_MAX:
                z_patch_arr, _, z_amask_arr = sample_target(
                    image, self.state,
                    self.params.template_factor,
                    output_sz=self.params.template_size
                )
                self.z_dict1 = self.preprocessor.process(z_patch_arr, z_amask_arr)
