...
from lib.train.dataset.competition_uav import CompetitionUAV
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val",
                        "COCO17", "VID", "TRACKINGNET", "COMPETITION_UAV", "EXTERNAL_UAV"]
        if name == "LASOT":
        ...
        if name == "COMPETITION_UAV":
            import os
            from lib.train.admin.local import EnvironmentSettings
            env = EnvironmentSettings()
            datasets.append(CompetitionUAV(
                root=env.data_dir,                                               # ← original data (not data_small)
                manifest_path=os.path.join(env.metadata_dir, 'contestant_manifest_train225.json'),  # ← 225 train only
                image_loader=image_loader
            ))
        if name == "EXTERNAL_UAV":
            from lib.train.dataset.external_uav import ExternalUAV
            datasets.append(ExternalUAV(root=settings.env.external_uav_dir,
                       image_loader=image_loader))
    return datasets
